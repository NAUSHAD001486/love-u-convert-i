...
exports.cleanupOldResources = cleanupOldResources;
...
function isResourceOlderThan(resource, ttlSeconds) {
    try {
...
async function listResourcesByPrefix(prefix, resourceType, ctx) {
    const resources = [];
    ...
    do {
        try {
            ...
            const result = await cloudinary_1.default.api.resources(options);
...
async function deleteResources(publicIds, resourceType, ctx) {
    const requestId = ctx?.requestId || 'unknown';
    let deleted = 0;
    let failed = 0;
    const errors = [];
    ...
    for (let i = 0; i < publicIds.length; i += batchSize) {
        ...
        try {
            const result = await cloudinary_1.default.api.delete_resources(batch, {
                resource_type: resourceType,
            });
            if (result.deleted) {
            ...
            if (result.errors) {
                ...
                errors.push(...result.errors.map((e) => e.message || String(e)));
            }
...
async function cleanupOldResources(options = {}) {
    const { dryRun = true, prefix = 'love-u-convert', ttlSeconds = Number(process.env.CLEANUP_TTL_SECONDS || 28800), } = options;
    const ctx = options;
    ...
    try {
        ...
        const candidates = allResources.filter(resource => isResourceOlderThan(resource, ttlSeconds));
        ...
        if (dryRun) {
            ...
            for (const candidate of candidates) {
                ...
                const ageSeconds = (Date.now() - createdAt.getTime()) / 1000;
                console.error(`[${requestId}] Would delete: ${candidate.public_id} (${candidate.resource_type}, age: ${Math.floor(ageSeconds)}s)`);
            }
            return result;
        }
        // Real deletion
